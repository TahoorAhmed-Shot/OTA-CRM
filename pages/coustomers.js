...
// TODO AFTER CATEGORY CHANGES STATE VALUE *
function coustomers({
  handelroles,
  tostSuccess,
  tostError,
  ...
  const onSearch = (e) => {
    ...
    if (getSearchData.length > 0) {
      let SearchData = coustomerFilterData.filter((item) => {
        return item.name.toLowerCase().includes(getSearchData.toLowerCase());
      });
      setcoustomerData(SearchData);
    } else {
      setcoustomerData(coustomerFilterData);
    }
  };
  ...
  let updateCoustomer = (currentUser) => {
    ...
    setValue({
      ...
      company_country: currentUser.company_country,
  ...
  let deleteCoustomer = (currentUser) => {
    ref.current.classList.add("blur-sm");
  ...
  const handelDelete = async (userId) => {
    let url = `${HOST}/api/admin/customers/${userId}`;
    let params = {
      method: "DELETE",
      headers: {
  ...
  const handelupdateCoustomer = async (userId) => {
    console.log(userId);
    let url = `${HOST}/api/admin/customers/${userId}`;
    let params = {
      method: "PUT",
            ...
            <form className="sm:pr-3 mb-4 sm:mb-0">
              ...
              <div className="mt-1 relative sm:w-64 xl:w-96">
                <input
                  ...
                  placeholder="🍳  Search for coustomer"
                  value={query}
                  onChange={onSearch}
                />
              </div>
          ...
          <table className="w-full text-sm text-left text-gray-500  dark:text-gray-400">
            <thead className="text-sm     dark:bg-gray-900 dark:text-gray-800">
              <tr className="border-b border-slate-500 ">
                <th scope="col" className="px-3 py-3 text-white">Images</th>
                ...
                ? coustomerData.map((user) => {
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{user.company_city}</td>
                        <td className="px-6 py-4">{user.company_phone}</td>
                        <td className="px-6 py-4">{user.company_email}</td>
                        <td className="px-6 py-4 text-center   ">
                          {" "}
                          <span className="bg-green-600 animate-pulse sm:px-1.5  px-2 justify-center text-green-200  font-normal text-center text-base  flex py-1 align-middle  rounded-2xl">
                            active
                          </span>{" "}
                          {/* <span className="bg-red-600 sm:px-1.5 px-2 flex justify-center text-red-300 font-normal text-center text-base  py-1 align-middle  rounded-2xl">
                            unactive
                          </span>{" "} */}
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            onClick={() => {
                              ...
                              updateCoustomer(user);
                            }}
                            className="text-white mx-1 mb-1 bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-200 font-medium rounded-lg text-sm  inline-flex items-center sm:px-3 sm:py-2 py-2 px-2 text-center"
                          >
                            <svg
            ...
            <div className="relative transform overflow-hidden rounded-lg bg-white text-left  shadow-2xl shadow-slate-600 transition-all sm:my-8 sm:w-full sm:max-w-lg">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                <button
                  onClick={() => {
                    settoggel(false);
                    handelupdateCoustomer(formValue.id);
                    ref.current.classList.remove("blur-sm");
                  }}
                  type="button"

...
function category({
  ...
  const onSearch = (e) => {
    ...
    if (getSearchData.length > 0) {
      let SearchData = categoryFilterData.filter((item) => {
  ...
  let UpdateCategory = (currentUser) => {
    ...
    setValue({
      ...
      team_lead: currentUser.team_lead,
      id: currentUser.id,
    });
  };

  let deleteCategory = (currentUser) => {
    ref.current.classList.add("blur-sm");
                ...
                ? categoryData.map((user) => {
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{user.team_lead}</td>
